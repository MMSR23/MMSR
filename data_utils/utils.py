...
def save_model(now_epoch, model, model_dir, scaler, optimizer, rng_state, cuda_rng_state, Log_file):
    ckpt_path = os.path.join(model_dir, f'epoch-{now_epoch}.pt')
    torch.save({'model_state_dict': model.module.state_dict(),  # Model
                'optimizer': optimizer.state_dict(),  # optimizer state
                'rng_state': rng_state,   # torch's random number status
                'cuda_rng_state': cuda_rng_state, # Random number status of torch.cuda
                'scaler_state':scaler.state_dict()}, ckpt_path)  # Status of amp and autocast
