...
# text-fusion (distributed)
def get_text_fusion_scoring(model, item_content, test_batch_size, args, local_rank):
    ...
    with torch.no_grad():
        for input_ids in item_dataloader:
            # text
            input_ids = input_ids.to(local_rank)
            item_scoring_text = model.module.text_encoder(input_ids)
            # mask
            text_mask_scoring = torch.narrow(input_ids, 1, args.num_words_title, args.num_words_title)
            # fusion
            if args.fusion_method in ['sum', 'concat', 'film', 'gated','sum_dnn', 'concat_dnn', 'film_dnn','gated_dnn']:
                ...
                item_scoring_CV = torch.zeros_like(item_scoring_text).to(local_rank)
...
# CV-fusion (distributed)
def get_LMDB_fusion_scoring(model, item_num, item_id_to_keys, test_batch_size, args, local_rank):
    model.eval()
    

    item_dataset = Build_Lmdb_Eval_Dataset(data=np.arange(item_num + 1), item_id_to_keys=item_id_to_keys,
                                           db_path=os.path.join(args.root_data_dir, args.dataset, args.lmdb_data),resize=args.CV_resize)
    test_sampler = SequentialDistributedSampler(item_dataset, batch_size=test_batch_size)
    item_dataloader = DataLoader(item_dataset, batch_size=test_batch_size, num_workers=args.num_workers,
                                 pin_memory=True, sampler=test_sampler)
    ...
    with torch.no_grad():
        for input_ids in item_dataloader:
            input_ids = input_ids.to(local_rank)
            item_scoring_CV = model.module.cv_encoder(input_ids)
            if args.fusion_method in ['sum', 'concat', 'film', 'gated','sum_dnn', 'concat_dnn', 'film_dnn','gated_dnn']:
                if 'resnet' in args.CV_model_load or 'RN50' in args.CV_model_load:
                    item_scoring_CV = item_scoring_CV.to(local_rank) # resnet
                    item_scoring_text = torch.zeros_like(item_scoring_CV).to(local_rank)
                else:
