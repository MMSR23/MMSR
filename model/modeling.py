...
class CoAttention(BertPreTrainedModel):
    def __init__(self, config,args):
        ...
        self.token_type_embeddings = nn.Embedding(2, args.embedding_dim)

        self.apply(self.init_bert_weights)

    def forward(self, lang_feats, lang_attention_mask, visn_feats, visn_attention_mask, local_rank):
        import numpy as np
        ...
        lang_att_output = lang_feats + self.token_type_embeddings(torch.zeros_like(lang_attention_mask).long().to(local_rank))
        visn_att_output = visn_feats + self.token_type_embeddings(torch.ones_like(visn_attention_mask).long().to(local_rank))
